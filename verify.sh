...
#   1) JDK 21 확인  2) ./gradlew test 가 25개 테스트 전부 통과  3) bootRun 으로 :8080 기동·응답
#
...
if ./gradlew clean test --console=plain >/tmp/dingcotdd-test.log 2>&1; then
  ...
  if [[ "${TOTAL:-0}" -ge 25 ]]; then ok "테스트 25개 이상 실행됨"; else bad "기대한 25개보다 적게 실행됨 (${TOTAL:-0})"; fi
else
